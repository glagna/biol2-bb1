#!/usr/bin/env python3
"""
Export RD21 Wager Arena results from Firebase to CSV.

Usage:
    python3 export_rd21.py

Outputs two files in the current directory:
    rd21_results_YYYYMMDD_HHMM.csv     — full per-team grid (audit trail)
    rd21_leaderboard_YYYYMMDD_HHMM.csv — sorted ranking (for XP entry)
...
def main():
    print("Fetching /rd21 from Firebase...")
    try:
        data = fetch()
    ...
    if not teams:
        print("WARNING: no teams in /rd21/teams. Did you run the round?")
    if not key:
    ...
    # ============================================================
    detail_path = f"rd21_results_{timestamp}.csv"
    with open(detail_path, "w", newline="") as f:
        w = csv.writer(f)
        # Header rows
        w.writerow(["RD21 Wager Arena Results", f"exported {datetime.now().isoformat(timespec='seconds')}"])
    ...
    # ============================================================
    lb_path = f"rd21_leaderboard_{timestamp}.csv"
    with open(lb_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["RD21 Leaderboard", f"exported {datetime.now().isoformat(timespec='seconds')}"])
        w.writerow([])
