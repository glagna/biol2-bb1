#!/usr/bin/env python3
"""
Export RD19 Wager Arena results from Firebase to CSV.

Usage:
    python3 export_rd19.py

Outputs two files in the current directory:
    rd19_results_YYYYMMDD_HHMM.csv     — full per-team grid (audit trail)
    rd19_leaderboard_YYYYMMDD_HHMM.csv — sorted ranking (for XP entry)
...
def main():
    print("Fetching /rd19 from Firebase...")
    try:
        data = fetch()
    ...
    if not teams:
        print("WARNING: no teams in /rd19/teams. Did you run the round?")
    if not key:
    ...
    # ============================================================
    detail_path = f"rd19_results_{timestamp}.csv"
    with open(detail_path, "w", newline="") as f:
        w = csv.writer(f)
        # Header rows
        w.writerow(["RD19 Wager Arena Results", f"exported {datetime.now().isoformat(timespec='seconds')}"])
    ...
    # ============================================================
    lb_path = f"rd19_leaderboard_{timestamp}.csv"
    with open(lb_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["RD19 Leaderboard", f"exported {datetime.now().isoformat(timespec='seconds')}"])
        w.writerow([])
