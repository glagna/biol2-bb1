...
**Target URL:** `https://glagna.github.io/biol2-bb1/rd19_game/` (verified 2026-05-19 against the biol2-bb1 repo: rd15_game through rd18_game all use the `_game` suffix, so RD19 follows the same pattern.)
...
| `rd19-questions.json` | 8 LO3.5 outbreak questions; Q8 has `doublePayout: true` |
| `firebase.json` / `database.rules.json` | Carried from rd18_game template; unused for GH Pages deploy |
| `export_rd19.py` | Run after class to capture audit CSV of bets/answers/scores |

**Patches applied to the rd18_game clone (Claude, 2026-05-19):**
- All `rd18` references swapped to `rd19` across HTML, JSON, Python (URL paths, Firebase nodes, localStorage keys, fetch path).
- All `RD18` headers and brand strings swapped to `RD19` (page title is now "RD19 - The Wager Arena").
...
```bash
# 1. Copy the rd19_game folder into the repo
cp -r "/Users/giorgiolagna/Documents/Claude/Projects/BIOL2-S26/03_week_materials/week_08/rd19_game" ~/biol2-bb1/

# 2. Push to GitHub
cd ~/biol2-bb1
git add rd19_game
git commit -m "RD19 Outbreak Reps for S23 Wed May 20"
git push origin main

# 3. Wait ~1 min for GitHub Pages to publish, then verify
open https://glagna.github.io/biol2-bb1/rd19_game/
```
...
# Realtime Database -> rd19 -> delete the rd19 node entirely. New nodes
...
```bash
firebase --project biol2-bb1 database:remove /rd19 -y
```
...
```bash
cd "/Users/giorgiolagna/Documents/Claude/Projects/BIOL2-S26/03_week_materials/week_08/rd19_game"
python3 export_rd19.py
# Produces: rd19_results_2026-05-20.csv with per-team bets, answers, points per question
```
...
Source bank: `04_assessments/BIOL2_RD19_S23_Outbreak.md` (full rationales).
