# Firebase RTDB rules status for RD17 — Fri May 15 readiness
...
| Probe | Result |
|---|---|
| `PUT /rd17/state/phase = "probe"` (unauthenticated) | Succeeded |
| `PUT /rd16/state/phase = "probe"` (unauthenticated) | Succeeded |
| `GET /rd17/state` (unauthenticated) | Succeeded (read returned the value just written) |
| `GET /rd16/state` (unauthenticated) | Succeeded |

Both probe values were deleted immediately after the test. /rd17/state is back to null; /rd16/state retains the answerKey + currentQ from past RD16 games and is fine.
...
The RD client (`index.html`, `host.html`, `leaderboard.html`) does **not** call `signInAnonymously()`. It never authenticates. If we deploy the hardened rules tonight, every write from the game will fail because `auth.uid` will be null. RD17 would 100% break Friday morning.
...
The hardened rules now sitting in `database.rules.json` cover rd13 through rd17 with the same per-round structure (state writeable only by hosts, teams writeable by team or host, hosts node read-only). When you're ready for option 3, the rules file is paste-ready.

## Files in this folder relevant to rules

- `database.rules.json` — hardened rules covering rd13-rd17 + bb1 (do not deploy tonight)
