#!/usr/bin/env python3
"""
Export RD17 Wager Arena results from Firebase to CSV.

Usage:
    python3 export_rd17.py

Outputs two files in the current directory:
    rd17_results_YYYYMMDD_HHMM.csv     — full per-team grid (audit trail)
    rd17_leaderboard_YYYYMMDD_HHMM.csv — sorted ranking (for XP entry)
...
def main():
    print("Fetching /rd17 from Firebase...")
    try:
        data = fetch()
    ...
    if not teams:
        print("WARNING: no teams in /rd17/teams. Did you run the round?")
    if not key:
    ...
    # ============================================================
    detail_path = f"rd17_results_{timestamp}.csv"
    with open(detail_path, "w", newline="") as f:
        w = csv.writer(f)
        # Header rows
        w.writerow(["RD17 Wager Arena Results", f"exported {datetime.now().isoformat(timespec='seconds')}"])
    ...
    # ============================================================
    lb_path = f"rd17_leaderboard_{timestamp}.csv"
    with open(lb_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["RD17 Leaderboard", f"exported {datetime.now().isoformat(timespec='seconds')}"])
        w.writerow([])
