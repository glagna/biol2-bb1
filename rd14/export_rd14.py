#!/usr/bin/env python3
"""
Export RD14 Wager Arena results from Firebase to CSV.

Usage:
    python3 export_rd14.py

Outputs two files in the current directory:
    rd14_results_YYYYMMDD_HHMM.csv     — full per-team grid (audit trail)
    rd14_leaderboard_YYYYMMDD_HHMM.csv — sorted ranking (for XP entry)
...
def main():
    print("Fetching /rd14 from Firebase...")
    try:
        data = fetch()
    ...
    if not teams:
        print("WARNING: no teams in /rd14/teams. Did you run the round?")
    if not key:
    ...
    # ============================================================
    detail_path = f"rd14_results_{timestamp}.csv"
    with open(detail_path, "w", newline="") as f:
        w = csv.writer(f)
        # Header rows
        w.writerow(["RD14 Wager Arena Results", f"exported {datetime.now().isoformat(timespec='seconds')}"])
    ...
    # ============================================================
    lb_path = f"rd14_leaderboard_{timestamp}.csv"
    with open(lb_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["RD14 Leaderboard", f"exported {datetime.now().isoformat(timespec='seconds')}"])
        w.writerow([])
